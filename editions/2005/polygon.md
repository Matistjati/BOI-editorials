...
up with the diameter of about $N\cdot A/\pi$. Therefore, there may be no more
...
Thus, in the worst case, the first too big diameter is about $2\cdot N\cdot A/\pi$,
and we have to perform the binary search in a range of size $N\cdot A/ \pi$. If
...
to perform $O(log(N\cdot A/P))$ steps in the search, with $O(N)$ work to be done
for each step.

Altogether we have to perform about $N\cdot log(N\cdot A/P)$ operations, which is
