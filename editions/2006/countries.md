...
1. If the influence of city j is no more than the number s_i of
soliders in city i, then city i is marked as the capital of a kindgom.

2. Otherwise if there were several choices for j, then city i is
marked as the capital of a democracy.

3. Otherwise city i is marked as surrendering to city j.

Based on these marks, the output for each city i=1...N can be printed
as follows:

1. If city i is marked as a capital of a kingdom, print 'K'

2. If city i is marked as a capital of a democracy, print 'D'

3. Chase the chain of marks i surrenders to j, j surrenders to k, k
